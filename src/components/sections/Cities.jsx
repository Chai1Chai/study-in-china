        ...
        <div className="md:hidden mb-8 relative z-20">
          ...
          >
            <span className="text-[#636024] font-medium">
              {activeCityId 
                ? PROVINCES_DATA.find(p => p.id === activeCityId)?.name 
                : "Выберите город"}
            </span>
            <svg 
          ...
          <svg viewBox="0 0 867 610" className="absolute top-0 left-0 w-full h-full">
            {PROVINCES_DATA.map((prov) => (
              <path
                ...
                className={`transition-all duration-300 cursor-pointer ${
                  currentCity?.provincedID === prov.provincedID ? 'fill-[#8B1D1D]/40' : 'fill-transparent'
                }`}
              />
            ))}
            ...
            {PROVINCES_DATA.map((prov) => (
              <image
                ...
                width="24"
                height="24"
                className={`transition-all duration-300 cursor-pointerr ${currentCity?.id === prov.id ? 'scale-150 opacity-100' : 'scale-100 opacity-90 hover:opacity-100'
                }`}
